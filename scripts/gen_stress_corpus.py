...
def main(corpus_dir: str = "corpus-stress", with_fleet_shapes: bool = False) -> int:
    ...
    for tool, obj in payloads:
        ...
        # `manual=True`, as `terse capture` passes: each payload is one whole result. Without
        # it, payloads written back-to-back read as blocks of ONE legacy result and are
        # timing-grouped together by `policy generate`/`tune` (#380).
