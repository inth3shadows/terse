...
Deliberately scoped to detection only. A review pass found the fix opens a second hole:
#268's live cause scales with PROMPT LENGTH, and the diff arm's prompt is longer than its
control's, so the arm under test is systematically the arm that fails, and excluding those
calls per-arm while gating only on the POOLED share can let a real per-arm regression
publish as PASS. Closing that is a correlated-loss protection design question of its own
(#280), not a corollary of "detect no-content" — see #280 rather than this module for it.
"""
...
def test_dropeval_no_content_is_an_error_not_a_wrong_answer(monkeypatch, capsys):
    ...
    miss can only under-sell enabling a lossy drop tier, never over-sell it. Excluding
    instead would need the same correlated-loss protection tracked in #280 — under-selling
    is why this side doesn't need to wait for it."""
...
def test_every_renderer_describes_an_unanswered_call_the_same_way():
    ...
    for name, text in rendered.items():
        ...
        assert "transport loss" in text, name
