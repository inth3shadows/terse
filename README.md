...
| **token-optimizer-mcp** (ooples, ~456★) | Closest *mechanism* rival: lossless **Brotli** plus cache-and-retrieve-by-key, and its own `smart_read`/`smart_grep`/`smart_glob` replacements. Two structural differences (per its docs — **not installed and tested here**, unlike the rows above): it is **not a proxy** (the agent must call `optimize_text`, nothing is transparent to an existing server), and **Brotli output is not model-readable**, so it is eviction-to-cache — the content leaves context and returns only via a `get_cached` round trip. terse's compressed form is still *read directly* (see the fluency results). Same asterisk as headroom's retrieve-cache, on a different backend. |
| **Schema-side proxies** — Atlassian **mcp-compressor** (104★), **mcpproxy-go** (305★), **Tool Filter MCP** (`respawn-llc`, 36★) | A category, not one entrant: all shrink tool **definitions** at connect time (lossless deferred-disclosure, federation, filtering) and leave results untouched — complementary and stackable with terse (`terse proxy -- mcp-compressor -- <server>`). mcpproxy-go claims ~99% reduction and +43% accuracy, all of it on schemas. One caveat on mcp-compressor: an opt-in `--toonify` flag *does* reformat call **results** into TOON, so "schemas only" isn't strictly true — but that path is off by default and is a single static pass with no diffing, per-tool policy, or cross-call state, out-competed by terse's codec on that axis. |

Adoption honesty: terse is new (just published to PyPI, few/no stars); TOON (25.0k★) and headroom
(63.5k★) are far more established. (Star counts verified via the GitHub API, 2026-07-31.) terse's
...
Star counts appear above as context, never as a filter: this list keeps entrants with no
adoption at all, because relevance here is the *axis* a tool works on, not its popularity —
and by a popularity bar terse would not list itself. The running candidate queue, including
everything screened out and why, is
