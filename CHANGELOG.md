...
- **A `terse capture` payload is its own result, not a legacy envelope (#380).** It wrote
  ...
  alone, so `kb.search` and `search --server kb` stay two results. An existing envelope
  keeps its first sighting's identity: an old hand capture and a pre-#148 proxy block are
...
- **A scan row can say where its ledger rows land (#397).** `parse_proxy_opts` now reads
  ...
  ledger; an entry pointed at another file writes real records the report never sees, which
  until now was indistinguishable from an entry that wrote none.
...
- **`terse fluency --codec-verdict` pre-flights every model and refuses the run if one
  cannot express a container tool argument (#403).** The first live run spent 57 minutes
  scoring `deepseek-v4-pro` at exactly 0% on the RAW control arm: it sends a `deref` answer
  as a JSON string (`"[\"x\"]"` for `["x"]`), right content, wrong type, identically on
  both arms. Five of six cells rendered UNRESOLVED as `control arm failed`, naming nothing.
  Before any corpus question, each model now answers two fixed `deref` questions — a
  nested array and a nested object, generated by the same question generator and sent
  through the same request function as a real trial — three scorable times each, and every
  ...
  stringified answer is still a miss; the new `encodes_as_json_string` only names it.
