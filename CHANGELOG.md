...
- **`terse fluency --codec-verdict` pre-flights every model and refuses the run if one
  ...
  Before any corpus question, each model now answers two fixed `deref` questions — one
  array, one object, generated by the same question generator and sent through the same
  request function as a real trial — three scorable times each, and every answer must
  match. A failure exits 2 with each model's reason on stderr (`sends container arguments
  as a JSON STRING`, `answered without calling terse.record_answer`, or a backend that
  never answered) and writes no report. The run is refused rather than continued without
