...
def codec_unresolved_reasons(rows: list[dict[str, Any]], excluded_from_group: int = 0,
    ...
    for arm, rate in _codec_low_call_arms(rows):
        ...
        # "in this run", not a bare rate: the same (model, payload, question) measured 29%
        # and then 100% twice on unchanged answering code (#412), and the unqualified
        # sentence was already being cited as a property of the model.
        reasons.append(f"{arm} arm delivered {shown} of its answers through the tool call "
                       f"in this run (a property of the run, not the model), need "
                       f"{_CODEC_MIN_CALL_RATE:.0%} — the run cannot show a value "
