...
def _safe_ask(answerer: Answerer, system: str, user: str) -> str | None:
    ...
    questions on every path (`_answerable`) for exactly that reason; with None those
...
def _ask_n(answerer: Answerer, system: str, user: str,
    ...
    either the model was never asked (transport failure, #263) or it was asked and produced
    no content at all (#268). Callers carry the count into the row so the report can decline
    to publish an accuracy for a backend that did not answer.

    Note what the count does NOT do on its own: `_form_stats` removes these from that arm's
    denominator, which is only safe while the losses are uncorrelated with the arm. See
    `report._unmeasured`'s per-arm trigger for the case where they are not."""
    ok = fails = 0
